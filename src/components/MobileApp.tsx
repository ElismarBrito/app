...
export const MobileApp = ({ isStandalone = false }: MobileAppProps) => {
    ...
    const setup = async () => {
      ...
      const dialerListener = PbxMobile.addListener('dialerCallStateChanged', async (event: any) => {
        ...
        try {
          ...
          // CORREÇÃO: Se a chamada foi atendida (ACTIVE), registrar answered_at
  ...
  const handleStopCampaign = async () => {
    try {
      ...
      // CORREÇÃO: Inclui answered_at para calcular duração corretamente
      const { data: activeCalls, error: fetchError } = await supabase
        .from('calls')
        .select('id, start_time, status, answered_at')
        .eq('device_id', deviceId)
      ...
      if (fetchError) {
        ...
        const updates = activeCalls.map(async (call) => {
          ...
          // CORREÇÃO: Calcular duração APENAS se a chamada foi ATENDIDA (tem answered_at)
          if (call.answered_at) {
            const answeredAt = new Date(call.answered_at).getTime();
            const duration = Math.floor((Date.now() - answeredAt) / 1000);
            updateData.duration = duration;
            console.log(`⏱️ [handleStopCampaign] Chamada ${call.id} foi atendida - duração: ${duration}s`);
          } else {
            // Chamada não atendida = duração 0
            updateData.duration = 0;
            console.log(`⏱️ [handleStopCampaign] Chamada ${call.id} não foi atendida - duração: 0s`);
          }
